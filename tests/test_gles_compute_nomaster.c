/*
 * NeoGPU GLES Compute Test - Using card node without master
 *
 * Uses DRM card node but skips drmSetMaster for compute-only workload.
 *
 * Build on Pi:
 * cd ~/001/neogpu
 * gcc -O3 -march=armv8-a+simd+fp16 -mtune=cortex-a72 -ffast-math \
 *     -funroll-loops -DNDEBUG \
 *     tests/test_gles_compute_nomaster.c -o /tmp/test_gles_compute_nomaster \
...
int main(void) {
    printf("=== NeoGPU GLES Compute (No Master) ===\n\n");
    
    // Open card node WITHOUT setting master
    ...
    printf("Opened card node (no master)\n");
