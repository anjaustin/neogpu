...
static void run_cpu_projections(const LayerSpec* spec, float* hidden, 
    ...
    printf("    run_cpu: hidden=%p, weights[0]=%p, outputs[0]=%p\n", (void*)hidden, (void*)weights[0], (void*)outputs[0]);
    fflush(stdout);
    /* q_proj: H x H */
    printf("    calling q_proj...\n");
    fflush(stdout);
    hs_ml_ternary_f32_proj(outputs[0], hidden, weights[0], spec->H, spec->H);
    printf("    q_proj done, outputs[0][0]=%f\n", outputs[0][0]);
    fflush(stdout);
    /* k_proj: kv x H */
    printf("    calling k_proj...\n");
    fflush(stdout);
...
int main(void) {
    ...
    /* Skip V3D init for now - just test CPU path */
