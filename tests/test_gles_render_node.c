/*
 * NeoGPU GLES Compute Test - Using render node
 *
 * Uses DRM render node for headless compute (no master needed).
 *
 * Build on Pi:
 * cd ~/001/neogpu
 * gcc -O3 -march=armv8-a+simd+fp16 -mtune=cortex-a72 -ffast-math \
 *     -funroll-loops -DNDEBUG \
 *     tests/test_gles_render_node.c -o /tmp/test_gles_render_node \
...
int main(void) {
    printf("=== NeoGPU GLES Compute (Render Node) ===\n\n");
