...
static u32 hs_ipc_expected_payload_len(u8 op) {
    switch ((OpCode)op) {
        case OP_SET_RECORD_MASK:  return 4;
        case OP_SET_CHAN_BUDGET:  return 8;
        case OP_SET_BLOCK_POLICY: return 2;
        case OP_QUERY_STATS:
        case OP_QUERY_FABRIC:
        case OP_FENCE:
            return 0;
...
static int hs_ipc_handle_req(HSIpcServer* srv, int cfd, const HSIpcHdr* hdr) {
    ...
    u32 expect = hs_ipc_expected_payload_len(req->op);
    if (expect == 0xFFFFFFFFu || req->payload_len != expect || req->payload_len > HS_PAYLOAD_SIZE) {
